...
import { View, Text, FlatList, ScrollView,Image,StyleSheet, Platform ,TouchableOpacity, TouchableWithoutFeedback, Keyboard ,SafeAreaView } from 'react-native';
import { useRoute, useNavigation  } from '@react-navigation/native';
const AnimeDetail = () => {
  const route = useRoute();
  const { animeId } = route.params;
  const textArray = ['呪術廻戦 渋谷事変', 'ダークファンタジー', '総合評価', '4.1'];
  const textArray2 = ['タイトル', 'あああああああああああああああああああああああああああああああああああああああああああああああああ', '呪術廻戦', '4.1'];
  const navigation = useNavigation();
  const handleSubmit = () => {
    navigation.navigate('Review');srijb
  };
  const reviewSubmit = () => {
...
<TouchableOpacity style={styles.backLink} onPress={() => navigation.navigate('AnimeList')}>
  ...
  <Text></Text>
      ...
      <Image style={styles.anime_image}
